...
load('prova_path.mat');%load(['original_pathRouting',int2str(n_relay),'.mat']);
...
for period = range
    ...
    if save_flag == 1
        savefig(['Routing/',dir,'/adaptiveROVpath_ROVperiod',int2str(ROV_period),ack,'.fig'])
        saveas(gcf,['Routing/',dir,'/adaptiveROVpath_ROVperiod',int2str(ROV_period),ack,'.png']);
        save(['Routing/',dir,'/adaptiveROVpath_ROVperiod',int2str(ROV_period),'.mat']);
    end
