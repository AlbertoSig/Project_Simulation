...
%    TDMA_matrices(:,:,i) = importdata(['bruteForceAlg2/matlab_matrix',int2str(i),'.dat']);
%    CTR_del(i) = importdata(['bruteForceAlg2/CTR_del',int2str(i),'.txt']);
%    ROV_del(i) = importdata(['bruteForceAlg2/ROV_del',int2str(i),'.txt']);
% end
% sum_del = CTR_del + ROV_del;

load('delay_matrix2.mat');
