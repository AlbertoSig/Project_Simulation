...
range = 1:1:60;
save_flag = 1;
dir = 'bruteForceMatricesSinPath/TDMApipeline(DESERTnewSlotDuration)/';%%migliore%ZigZag
...
dir = 'bruteForceMatricesSinPath/TDMApipeline(DESERTstandardSlotDuration)'; %ZigZag
load(['PosBasedRouting/',dir,'/figure/simPosBasedRt_NewPath.mat']);
%max_ROV_period_theo = slot_duration*3;
%rmse
figure(1);
...
dir = 'bruteForceMatricesSinPath/TDMApipeline(matlab)';
load(['PosBasedRouting/',dir,'/figure/simPosBasedRt_NewPath.mat']);
%max_ROV_period_theo = slot_duration*3;
%rmse
figure(1);
...
dir = 'bruteForceMatricesSinPath/TDMApipeline(MatirxForSinPathDesert)';
load(['PosBasedRouting/',dir,'/figure/simPosBasedRt_NewPath.mat']);
%max_ROV_period_theo = slot_duration*3;
%rmse
figure(1);
semilogy(range, adap_rmse,'-gh','MarkerSize',8);
grid on;
hold on;
%semilogy(ones(1,2)*max_ROV_period_theo,[10^-5 , 10^5],'--k');
title('RMSE');
...
%ROV packet delivery ratio
figure(2)
plot(range, adap_ROV_pdr,'-gh','MarkerSize',8);
grid on;
hold on;
%plot(ones(1,2)*max_ROV_period_theo,[10^-5 , 10^5],'--k');
axis([1 60 0 1]);
...
%CTR packet delivery ratio
figure(3)
plot(range, adap_CTR_pdr,'-gh','MarkerSize',8);
grid on;
hold on;
%plot(ones(1,2)*max_ROV_period_theo,[10^-5 , 10^5],'k--');
axis([1 60 0 1]);
...
%ROV packet delivry delay
figure(4);
plot(range, adap_ROV_pdd,'-gh','MarkerSize',8);
axis([1 60 0 75]);
grid on;
hold on;
%plot(ones(1,2)*max_ROV_period_theo,[10^-5 , 10^5],'k--');
...
%CTR packet delivery delay
figure(5)
plot(range, adap_CTR_pdd,'-gh','MarkerSize',8);
grid on;
hold on;
%plot(ones(1,2)*max_ROV_period_theo,[10^-5 , 10^5],'k--');
title('CTR packet delivery delay');
...
%CTR throughput
figure(6)
plot(range, adap_CTR_th,'-gh','MarkerSize',8);
grid on;
hold on;
%plot(ones(1,2)*max_ROV_period_theo,[10^-5 , 10^5],'k--');
title('CTR throughput');
...
%ROV throughput
figure(7)
plot(range, adap_ROV_th,'-gh','MarkerSize',8);
grid on;
hold on;
%plot(ones(1,2)*max_ROV_period_theo,[10^-5 , 10^5],'k--');
title('ROV throughput');
